...
defmodule Ex1018 do
  def distribute(0, []), do: nil
  def distribute(value, symbol) do
    [actual | rest] = symbol
    IO.puts("#{div(value, actual)} nota(s) de R$#{actual},00")
    distribute(rem(value, actual), rest)
  end
end

symbols = [100, 50, 20, 10, 5, 2, 1]
value = BcInput.input_as_integer()

Ex1018.distribute(value, symbols)
