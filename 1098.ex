...
defmodule Ex1098 do
  def print_recursively(i, _) when i > 2, do: nil

  def print_recursively(i, j) when round(j - i) > 3 do
    print_recursively(i + 0.2, j - 2.8)
  end

  def print_recursively(i, j) do
    IO.puts("I=#{BcParse.number_to_str(i)} J=#{BcParse.number_to_str(j)}")
    print_recursively(i, j + 1)
  end
end

Ex1098.print_recursively(0.0, 1.0)
