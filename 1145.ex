...
defmodule Ex1145 do
  def start(step, final) do
    print_lines(1, final, step)
  end

  def print_lines(actual, final, _step) when actual == final do
    IO.puts("#{actual} ")
  end
  def print_lines(actual, final, step) when (rem(actual, step) == 0) do
    IO.puts("#{actual}")
    print_lines(actual + 1, final, step)
  end
  def print_lines(actual, final, step) do
    IO.write("#{actual} ")
    print_lines(actual + 1, final, step)
  end
end

[step, last] = BcInput.input_as_integer_array()

Ex1145.start(step, last)


# Tried the approach using chunk_every then joining with a whitespace but the last line requires a
# whitespace before the breakline, because of this, I thought this approach better
